...
def main ():
	...
	for post in posts:		
		...
		poster_graph.node[post['ThreadRef']]['PostedOn'] = read_date(post['PostedOn'])
		posts_graph.add_node(post['ThreadRef'])
		posts_graph.node[post['ThreadRef']]['text'] = post["Content"]
		posts_graph.node[post['ThreadRef']]['PostedOn'] = poster_graph.node[post['ThreadRef']]['PostedOn']
	for comment in comments:
		...
		posts_graph.node[comment['_id']]['PostedOn'] = read_date(comment["PostedOn"])

	print ("created utility graphs")
	f = open("LIWC_DATA/utility_graphs/utility_graph_list.pkl", "w")
	pickle.dump([poster_graph,posts_graph,commenter_graph], f)
	f.close()
	print ("dumped utility graphs")
	comments = posts_graph.edges()
	#pdb.set_trace()
	comments = [(posts_graph.node[x]["PostedOn"], x, y, posts_graph.node[x]['text'], posts_graph.node[y]['text']) for x,y in comments]
	...
	while diff.days/7 >= 2:
		...
		while max_date <= end_date:
			...
			nx.write_gexf(d, "LIWC_DATA/utility_graphs/%d_digraph_%s_%s.gexf" % (diff.days/7, datetime.datetime.strftime(min_date, "%Y-%m-%d"), datetime.datetime.strftime(max_date, "%Y-%m-%d")))
			
			f = open("LIWC_DATA/utility_graphs/%d_posts_%s_%s.pkl" % (diff.days/7, datetime.datetime.strftime(min_date, "%Y-%m-%d"), datetime.datetime.strftime(max_date, "%Y-%m-%d")), "w")
			pickle.dump(nu_un,f)
			f.close()
			#pdb.set_trace()
			...
			nx.write_gexf(g, "LIWC_DATA/utility_graphs/%d_graph_%s_%s.gexf" % (diff.days/7, datetime.datetime.strftime(min_date, "%Y-%m-%d"), datetime.datetime.strftime(max_date, "%Y-%m-%d")))
